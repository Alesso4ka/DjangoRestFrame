...
## Последовательность действий tasks lesson-12
...
1. создаем папку nginx
2. создаем файл docker-compose.yml
3. Переносим папку fronded на уровень выше.
4. создаем файл Dockerfile

Проверяем в корневой папке, какие контейнеры есть с помощью команды: 
...
Повторяем команду:
    ...
    docker ps -a
Далее выполняем следующую команду, которая проверяет что работает из конкретного docker-compose
